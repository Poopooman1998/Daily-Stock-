...
async function main() {
  ...
  try {
    ...
    if (!Array.isArray(fmtqik) || !fmtqik.length) throw new Error('FMTQIK 回傳非陣列');
    const last = fmtqik[fmtqik.length - 1]; // 最後一筆 = 最近交易日
    // FMTQIK 欄位為英文:TAIEX / Change / TradeValue(同時保留中文備援)
    const close = num(pick(last, 'TAIEX', '發行量加權股價指數', '收盤指數'));
    const chg = num(pick(last, 'Change', '漲跌點數'));     // 已含正負號
    const prev = close - chg;
    const p = prev ? (chg / prev) * 100 : 0;
    const turnover = num(pick(last, 'TradeValue', '成交金額')) / 1e8; // 元 → 億元
    if (!close) throw new Error('FMTQIK 找不到指數欄位,實際鍵:' + Object.keys(last).join(','));
  ...
  try {
    ...
    if (!Array.isArray(inst) || !inst.length) throw new Error('BFI82U 回傳非陣列');
    // 容忍中英文欄位:單位名稱/Name、買賣差額/Difference
    const unitOf = (r) => String(pick(r, '單位名稱', 'Name', '名稱') || '');
    const diffOf = (r) => num(pick(r, '買賣差額', 'Difference', '買賣超', '差額'));
    const sumBy = (kw) => inst
      .filter((r) => unitOf(r).includes(kw))
      .reduce((a, r) => a + diffOf(r) / 1e8, 0); // 元 → 億
